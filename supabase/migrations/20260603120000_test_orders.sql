-- Stripe test-mode pickup orders (isolated from live orders).
...
create table public.test_order_items (
  id bigint generated always as identity primary key,
  order_id bigint not null references public.test_orders (id) on delete cascade,
...
create index test_orders_payment_status_idx on public.test_orders (payment_status);
create index test_orders_tracking_token_idx on public.test_orders (tracking_token) where tracking_token is not null;
create unique index test_orders_stripe_checkout_session_id_uidx
...
comment on column public.test_orders.stripe_mode is
  'Stripe payment environment used at checkout (test or live).';
comment on table public.test_order_items is 'Line items for Stripe test-mode pickup orders.';
comment on function public.create_paid_test_order_with_items is
  'Atomically creates a paid confirmed test order and its line items from a draft payload.';
...
create policy "Service role full access on test_order_items"
...
create policy "Admins full access on test_order_items"
  on public.test_order_items
  for all
  to authenticated
  using (public.is_admin())

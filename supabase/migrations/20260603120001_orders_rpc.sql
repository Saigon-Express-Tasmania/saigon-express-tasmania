...
comment on function public.create_pending_catering_order(jsonb, jsonb) is
  'Creates a catering order in pending status (no payment). Guest orders may omit customer_account; admin quotes later and customer pays when status allows.';
...
-- Atomically cancels a pending catering order: row lock, validation, archive, delete.
-- Prevents races with admin status updates (SELECT FOR UPDATE) and rejects paid orders.

-- Support store pickup for catering quote orders (create_pending_catering_order).
...
begin
  ...
  if v_requested_fulfillment_method = 'pick_up'::public.order_fulfillment_type then
    v_requested_pick_up_store_id := coalesce(v_requested_pick_up_store_id, v_store_id);
    if v_requested_pick_up_store_id is null then
      raise exception 'Pickup store is required';
    end if;
    v_store_id := coalesce(v_store_id, v_requested_pick_up_store_id);
    v_shipping_address := coalesce(nullif(v_shipping_address, ''), 'In-store pickup');
    v_shipping_city := coalesce(nullif(v_shipping_city, ''), 'N/A');
    v_shipping_postal_code := coalesce(nullif(v_shipping_postal_code, ''), '0000');
  elsif v_shipping_address = '' or v_shipping_city = '' or v_shipping_postal_code = '' then
...
comment on function public.create_pending_catering_order(jsonb, jsonb) is
  'Creates a catering order in pending status (no payment). Supports delivery or store pickup. Guest orders may omit customer_account.';

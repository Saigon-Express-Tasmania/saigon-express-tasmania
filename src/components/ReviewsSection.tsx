...
function ReviewCard({ reviewerName, rating, reviewText, location }: ReviewCardProps) {
   return (
      <div className="flex-shrink-0 w-80 bg-stone-900 border border-stone-800 rounded-2xl p-6 flex flex-col gap-4 mx-3 select-none">
         ...
         <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
               <p className="text-white font-semibold text-sm leading-tight truncate">{reviewerName}</p>
               {location && <p className="text-stone-400 text-xs mt-0.5 truncate">{location}</p>}
            </div>
            {/* Google G badge */}
         ...
         {/* Review text */}
         <p className="text-stone-300 text-sm leading-relaxed line-clamp-4 flex-1">&ldquo;{reviewText}&rdquo;</p>
      </div>
   );
}
   ...
   return (
      <section className="py-20 bg-stone-950 overflow-hidden">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 text-center">
            {/* Eyebrow */}
            <p className="text-red-500 text-xs font-bold tracking-widest uppercase mb-3">What Our Customers Say</p>
            ...
            <div className="flex items-center justify-center gap-3 mt-4">
               ...
               <span className="text-stone-400 text-sm">· {reviews?.length ?? 0} verified reviews</span>
            </div>
         </div>

...
export const SELF_DELIVERY_ORIGIN_KEY = "self_delivery_origin";

export const DEFAULT_SELF_DELIVERY_ORIGIN = "Hobart 7000";
...
export function parseSelfDeliveryOrigin(raw: string | undefined): string {
  const trimmed = raw?.trim();
  return trimmed || DEFAULT_SELF_DELIVERY_ORIGIN;
}

export function serializeSelfDeliveryFee(fee: SelfDeliveryFee): string {
  return JSON.stringify({
    gas_price: fee.gas_price,
    truck_engine_volume: fee.truck_engine_volume,
    maintenance_buffer: fee.maintenance_buffer,
    profit_margin: fee.profit_margin,
  });
}

/** Rough km/L estimate from engine displacement (capped at a sensible floor). */

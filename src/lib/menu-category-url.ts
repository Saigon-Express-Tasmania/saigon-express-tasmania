...
export function menuCategoryUrlParam(
...
export function resolveMenuCategoryFromUrlParam(
...
export function buildMenuCategoryQuery(
  categoryName: string,
  allLabel: string,
  categories: Pick<SiteCategory, "id" | "alias" | "name">[],
): string {
  ...
  const alias = menuCategoryUrlParam(categories, categoryName);
  if (!alias) return "";

...
const BENEFITS = [
  { icon: Zap, title: "Order in One Tap", desc: "Menu always ready — order faster than searching a browser." },
  { icon: Star, title: "Exclusive Deals", desc: "App-only promotions and loyalty rewards before they run out." },
  { icon: MapPin, title: "Find Stores Instantly", desc: "View all 8 Tasmania locations with hours and directions." },
  { icon: Smartphone, title: "No App Store Needed", desc: "Installs directly from your browser. Zero storage used." },
];

const STEPS_IOS_SAFARI = [
  { num: "1", text: "Open saigonexpress.com.au in Safari (not Chrome or Firefox)." },
  { num: "2", text: "Tap the Share button ⬆ at the bottom centre of Safari — it looks like a box with an arrow pointing up." },
  { num: "3", text: "Scroll down in the share sheet and tap \"Add to Home Screen\" (with a ⊕ icon)." },
  { num: "!", text: "In the name field, make sure it says Saigon Express. If it shows something else, clear it and type Saigon Express." },
  { num: "✓", text: "Tap Add in the top-right corner — the Saigon Express icon appears on your home screen!" },
];

const STEPS_IOS_FIREFOX = [
  { num: "1", text: "Open saigonexpress.com.au in Firefox." },
  { num: "2", text: "Tap the ⋯ menu button at the bottom-right of the screen." },
  { num: "3", text: "Tap Share from the menu that appears." },
  { num: "4", text: "In the iOS share sheet, tap \"View More\" then tap \"Add to Home Screen\"." },
  { num: "!", text: "In the name field, check it says Saigon Express. If not, clear it and type Saigon Express." },
  { num: "✓", text: "Tap Add — the Saigon Express icon appears on your home screen!" },
];
...
function StepCard({ step }: { step: { num: string; text: string } }) {
  ...
  return (
    <div className={`flex gap-4 p-4 rounded-xl border ${isCheck ? "border-green-200 bg-green-50" : isWarn ? "border-amber-200 bg-amber-50" : "border-border bg-card"}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${isCheck ? "bg-green-500 text-white" : isWarn ? "bg-amber-400 text-white" : "bg-primary text-white"}`}>
        {step.num}
      </div>
...
export default function GetTheApp() {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText("https://www.saigonexpress.com.au/get-the-app").then(() => {
      setCopied(true);
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Portal bar */}
      <div className="border-b border-border/50 bg-foreground text-background py-2">
        <div className="container flex items-center justify-between">
          <span className="text-xs text-background/50">Staff Access</span>
          <div className="flex items-center gap-4">
            {PORTAL_LINKS.map(l => (
              <Link key={l.href} href={l.href} className="text-xs text-background/60 hover:text-background transition-colors">{l.label}</Link>
            ))}
          </div>
        </div>
      </div>
      ...
      <nav className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border/50">
        <div className="container flex items-center justify-between h-16">
          ...
          <Link href="/menu">
            <button className="px-4 py-2 rounded-full text-sm font-semibold bg-primary text-white hover:bg-primary/90 transition-colors">Order Now</button>
          </Link>
        </div>
      </nav>
      ...
      <section className="py-20 text-center" style={{ background: "oklch(13% 0.008 30)" }}>
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="font-serif text-5xl lg:text-7xl font-bold text-white mb-4">Get the Saigon Express App</h1>
            <p className="text-white/55 text-lg max-w-xl mx-auto">
              Faster ordering. Exclusive deals. Always ready. Add Saigon Express to your home screen — no App Store, no storage used. Just one tap to open anytime.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-16" style={{ background: "oklch(98% 0.006 80)" }}>
        <div className="container">
          <div className="text-center mb-12">
            <div className="text-xs font-bold tracking-[0.2em] uppercase mb-3 text-primary">HOW TO INSTALL</div>
            <h2 className="font-serif text-4xl font-bold text-foreground">Add to Your Home Screen</h2>
            <p className="text-muted-foreground mt-3">Choose your device and browser below.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0 }}
              ...
              <div className="p-5 border-b border-border" style={{ background: "oklch(13% 0.008 30)" }}>
                <div className="text-2xl mb-1">🍎</div>
                <div className="font-bold text-white">iPhone — Safari</div>
                <div className="text-xs text-white/50 mt-1">Recommended · 3 steps</div>
              </div>
              <div className="p-5 space-y-3">
                {STEPS_IOS_SAFARI.map((s, i) => <StepCard key={i} step={s} />)}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
              ...
              <div className="p-5 space-y-3">
                {STEPS_IOS_FIREFOX.map((s, i) => <StepCard key={i} step={s} />)}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
              ...
              <div className="p-5 space-y-3">
                {STEPS_ANDROID.map((s, i) => <StepCard key={i} step={s} />)}
              </div>
            </motion.div>
          </div>

          <p className="text-center text-sm text-muted-foreground mt-8">
            💡 <strong>Tip:</strong> Safari works best on iPhone — it installs in 3 steps instead of 5.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 bg-background">
        <div className="container max-w-lg mx-auto text-center">
          <h2 className="font-serif text-3xl font-bold text-foreground mb-3">Open This Page on Your Phone</h2>
          <p className="text-muted-foreground mb-8">Works on iPhone (Safari) and Android (Chrome) · No App Store needed</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a href={`https://wa.me/?text=${encodeURIComponent("Add Saigon Express Tasmania to your home screen: https://www.saigonexpress.com.au/get-the-app")}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
            <div className="md:col-span-2">
              ...
              <div className="mt-4 space-y-1">
                ...
                <div className="text-white/40 text-xs">Ph: 0416 036 016</div>
                <div className="text-white/40 text-xs">Level 2, 86 Collins St, Hobart TAS 7000</div>
              </div>
            </div>
            <div>

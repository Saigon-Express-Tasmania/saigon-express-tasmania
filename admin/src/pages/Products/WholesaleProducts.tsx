  ...
  const filteredProducts = useMemo(() => {
    ...
    const filtered = products.filter((p) => {
      if (
        ...
        !p.categoryIds.some((categoryId) => scopedCategoryIds.has(categoryId))
      ) {
        return false;
      }
      ...
      if (!matchesPublishedFilter(p.is_published ?? false, publishedFilter)) {
        return false;
      }
      if (!term) return true;
    ...
    <DashboardLayout title="Wholesale Products">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Publish {selectedCount} wholesale{' '}
              {selectedCount === 1 ? 'product' : 'products'}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This will set {selectedCount} wholesale{' '}
              {selectedCount === 1 ? 'product' : 'products'} to published and
              make {selectedCount === 1 ? 'it' : 'them'} visible on the
              storefront.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

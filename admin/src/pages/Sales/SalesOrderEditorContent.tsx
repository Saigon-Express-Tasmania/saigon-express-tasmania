...
export type SalesOrderEditorContentProps = {
...
export function SalesOrderEditorContent({
  dataset,
  orderType,
  layout,
  editingOrderId,
  ...
  const itemsTabLabel = `Items & totals${form.items.length > 0 ? ` (${form.items.length})` : ''}`;
  ...
  return (
    ...
    >
      <TabsList className="h-auto w-full shrink-0 flex-wrap justify-start gap-1 bg-muted/40 p-1">
        ...
        <TabsTrigger value="items">{itemsTabLabel}</TabsTrigger>
